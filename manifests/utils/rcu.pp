...
define orawls::utils::rcu(
  ...
  wls_rcu{ $rcu_prefix:
    ...
    statement    => "${oracle_fmw_product_home_dir}/bin/rcu -silent ${action} -connectString ${rcu_database_url} -dbUser ${rcu_sys_user} -dbRole ${rcu_sys_role} -honorOMF -schemaPrefix ${rcu_prefix} ${components} -f < ${download_dir}/rcu_passwords_${fmw_product}_${rcu_action}_${rcu_prefix}.txt",
    os_user      => $os_user,
    oracle_home  => $oracle_fmw_product_home_dir,
    sys_user     => $rcu_sys_user,
    sys_password => $rcu_sys_password,
    sys_role     => $rcu_sys_role,
